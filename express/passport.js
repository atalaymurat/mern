  ...
  new LocalStragety(
    ...
    async (req, email, password, done) => {
      try {
        ...
        if (!user.active) {
          console.log("[PASS-LCL] Kullanıcı pasif durumda!");
          return done(null, false, {
            message: "Hesabınız pasif durumda. Lütfen yöneticinize başvurun.",
          });
        }
        console.log("[PASS-LCL] Email, şifre, doğrulama ve aktiflik OK!");
        done(null, user);
      } catch (error) {
        done(error, false);
      }
    ...
    async (req, payload, done) => {
      try {
        ...
        if (!user) {
         ...
         if (!user.active) {
          console.log('[PASS-JWT] Kullanıcı pasif durumda')
          return done(null, false, { message: 'Kullanıcı pasif durumda' })
        }
